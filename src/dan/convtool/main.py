...
class ConvTool(BaseTool):
    ...
    def run(self):
        ...
        layer_index_dict = {name: index for index, name in enumerate(new_solver.layer)}
    ...
    def _generate_new_proto(cls, ori_solver, rank_spec):
        ...
        new_solver = caffepb2.NetPrameter()
        ...
        if not set(self.mode["compression_specification"]) < conv_layer_names:
            logger.error("Conv layers do not exist: < %s >. \t"
                         ...
                         ', '.join(set(self.mode["compression_specification"])
                                   - conv_layer_names))
        layer_index_dict = {name: index for index, name in enumerate(self.ori_solver.layer)}
        self.rank_spec = {}
        for layer_name, times in self.mode["compression"].iteritems():
            try:
                times = int(times)
            except Exception as e:
                print e
